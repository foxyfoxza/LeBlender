angular.module("umbraco").controller("LeBlender.Dialog.Parameterconfig.Controller",
    function ($scope, assetsService, $http, dialogService, mediaHelper, $timeout, LeBlenderRequestHelper, umbPropEditorHelper) {
        ...
        $scope.validateProperty = function (property) {
            delete property.validation.patternFailed;
            delete property.validation.requiredFailed;

            if (property.validation.mandatory && !property.value) {
                property.validation.requiredFailed = true;
            }
            ...
            if (property.validation.regex && property.validation.regex != "") {
                ...
                if (regEx.test(property.value) == false) {
                    property.validation.patternFailed = true;
                }
            }

            return !(property.validation.patternFailed || property.validation.requiredFailed);
        }
        ...
        $scope.validate = function () {
            ...
            $scope.model.isValid = !failed && $scope.parameterconfigForm.$valid;
            return !failed;
        }
        ...
        $scope.ResolveProperties = function (editorKeys, editors, item) {
            ...
            //sync the Editor's validation settings to this object
            _.each(editors, function (editor, editorIndex) {
                var property = item[editor.alias];
                property.validation.mandatory = editor.mandatory;
                property.validation.regex = editor.validation;
            });
        }

        $scope.syncProperties = function () {
            var valueEditorKeys = [];
            _.each($scope.config.editors, function (editor, editorIndex) {
